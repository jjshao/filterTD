...
stats <- num_obs_per_tag(animal_tag_data, id_col_name, graph=FALSE)
```
...
for(x in vec_species) {
        cat("### Graph for", x, " \n")
        num_obs_per_tag_ani <- num_obs_per_tag(animal_merged, id_col_name, graph=TRUE,
                                               grouping=x,
...
percentage of data. 
``` {r echo = FALSE}
sync_hpe <- hpe_quantiles(sync_tag_data, hpe_col_name, hpem_col_name)
kable(sync_hpe)
```
...
sync_summary <- sync_summary(sync_tag_data, hpe_col_name, sync_hpe_vec)
kable(sync_summary) 
```
...
receiver_detections(sync_tags=sync_tag_data, sync_hpe_col_name=hpe_col_name,
...
``` {r echo = FALSE, results = 'asis', fig.width=10, fig.height=10, fig.fullwidth=TRUE, warning = FALSE}
receiver_detections(sync_tags=sync_tag_data_250, sync_hpe_col_name=hpe_col_name,
                    lat_col_name=lat_col_name, long_col_name=long_col_name, id_col_name=id_col_name)
```

## Graphs of Animal Positions
``` {r echo = FALSE, results = 'asis', fig.width=10, fig.height=15, fig.fullwidth=TRUE, warning = FALSE}
for(x in ani_tags) {
  cat("### Graph for", x, " \n")
  animal_positions(animal_tags=animal_tag_data, ani_hpe_col_name=hpe_col_name,
