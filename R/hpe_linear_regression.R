...
#' # Example
#' sync_linreg <- hpe_linear_regression(dummy_sync, "HPE", "HPEm", 20, "Id", "X", "Y")
#'
#' @export
#'
#' @import ggplot2
...
hpe_linear_regression <- function(sync_tags, hpe_col_name, hpem_col_name,
  ...
  print(sync_tags %>% ggplot(aes(!!sym(hpe_col_name), error.m)) +
          ...
          geom_abline(intercept = coef(res3)[1],
                      slope = coef(res3)[2],
                      colour = "white",
                      linewidth = 1) +
          geom_abline(intercept = coef(res3)[1],
          ...
          annotate("text", x = -Inf, y = Inf,
                   ...
                   label = label_text, size= 6, parse=TRUE, color = 'red'))
