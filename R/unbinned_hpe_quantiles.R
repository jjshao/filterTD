...
#' and output scatterplots that showed HPE values vs HPEm.
#'
...
#' sync_hpe2 <- unbinned.hpe.quantiles(sync_tag_data, "HPE", "HPEm", quantiles)
#'
#' @export

unbinned.hpe.quantiles <- function(sync_tags, hpe_col_name, hpem_col_name, quantiles=NULL) {
  ...
  for (x in quantile_data) {
    ...
    # Plot
    plot(as.numeric(remaining_tags$HPE), remaining_tags$HPEm, main=paste("HPE vs HPEm at Q", x),
