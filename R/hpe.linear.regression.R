...
hpe.linear.regression <- function(sync_tags, hpe_col_name, hpem_col_name,
  ...
  binMean <- sync_tags %>%
    ...
    summarize(bin_mean = mean(error.m, na.rm = TRUE), bin_num = length(error.m), Xe_Sd = sd(Xe), Ye_Sd = sd(Ye),
              RMS2d = 2*sqrt((Xe_Sd)^2 + (Ye_Sd)^2), avgHPE = mean(HPE))
  ...
  print(sync_tags %>% ggplot(aes(HPE, error.m)) +
    geom_point() +
    ...
    annotate("text", x = 5, y = hpe_val, label = lm.eqn(res3), size= 6, parse=TRUE, color = 'red'))
}
