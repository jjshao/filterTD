...
prelim_report <- function(Tags, SyncTags=NULL, Metadata, ani_tags, sync_tags,
  ...
  if (!missing(SyncTags)) {
    ...
    tags_subset <- split_tag_data(tags = all_tags,
  ...
  report_file <- system.file("reports", "prelim.report.Rmd", package = "filterTelemetry")

  rmarkdown::render(report_file, params = list(tags=tags_subset,
                                               metadata = Metadata,
